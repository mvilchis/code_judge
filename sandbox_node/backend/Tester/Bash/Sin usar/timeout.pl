...
# Some lines are added by Mohammad Javad Naderi <mjnaderi@gmail.com> 
# (indicated by "# added by mjnaderi") to enable using EasySandbox 
# in Sharif Judge online judge (https://github.com/mjnaderi/Sharif-Judge)
# July, 29, 2013
# Enable EasySandbox:  "--sandbox"
# Disable EasySandbox: "-nosandbox"
...
my $ticklimit = undef;
my $memlimit = undef;
my $sandbox = undef; # added by mjnaderi
my $outlimit = undef; # size limit of output file // added by mjnaderi
my $strpat = undef;
...
my $frequency = 20; # changed by mjnaderi from 10 to 20
# if we debug
my $debug = '';
...
GetOptions(
	'timelimit|t=f'=>\$timelimit, # changed by mjnaderi (original: 'timelimit|t=i'=>\$timelimit,). Now we can use real values for timelimit
	...
	'sandbox|a=s'=>\$sandbox, # added by mjnaderi
	'outlimit|l=i'=>\$outlimit, # added by mjnaderi
...
my $id_str = 'SHJ_'; #changed by mjnaderi
...
unless ($blackbox_pid){
	...
	local $" = " ";
	if (defined $sandbox) {                           # added by mjnaderi
		$ENV{'LD_PRELOAD'}=$sandbox;   # added by mjnaderi
	}                                            # added by mjnaderi
	exec @ARGV or die "Couldn't exec @ARGV: $!";
	if (defined $sandbox) {                           # added by mjnaderi
		$ENV{'LD_PRELOAD'}="";                   # added by mjnaderi
	}                                            # added by mjnaderi
}
...
while ($status eq 'wait'){
	...
	eval {
		local $SIG{'ALRM'} = sub {
			...
			$sizeinfo = -s 'out'; # size of output file // added by mjnaderi
			$size_err_info = -s 'err'; # size of err file // added by mjnaderi
...
sub limits_exceeded
{
	...
	# Sometimes the controlling process may inherently hang up.  Then we don't interrupt it.
		...
		return 'MEM';
	}elsif (defined $outlimit && ($sizeinfo > $outlimit || $size_err_info > $outlimit) ){ # added by mjnaderi
		return 'OUTSIZE';
	}
	return undef;
}
