...
wallclock=30 #Segundos
cpu=1			#Segundos
memory=8192 #Kilobytes
disk=1024   #Kilobytes

if [ ! -r $input ]; then
	# Test does not expect input
	python ../sample2.py build/${testexe} ${wallclock} ${cpu} ${memory} ${disk} > ${output} 2> ${error}
else
	python ../sample2.py build/${testexe} ${wallclock} ${cpu} ${memory} ${disk} < ${input} > ${output} 2> ${error}
fi
